...
for json_file in os.listdir(json_dir):
    ...
    weather_name = data['info']['weather']
    weather_count[weather_name] += 1  # 해당 label의 카운트를 증가

# 결과 출력
print("Class count per label:")
for weather_name, count in weather_count.items():
    print(f"{weather_name}: {count} occurrences")
